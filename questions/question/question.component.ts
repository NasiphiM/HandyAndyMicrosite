...
export class QuestionComponent  implements OnInit {
  ...
  nextQuestion() {
    ...
    if (this.questions[this.currQuestion - 1].questionId == 1 && this.questions[this.currQuestion - 1].answer.answerText?.toLowerCase() == "no") {
      ...
      this.currQuestion = 1;
      this.getQuestion(this.currQuestion);
      this.route.navigate(['/capture-details']);
      return;
    }

    if (this.questions[this.currQuestion - 1].questionId == 2 && this.questions[this.currQuestion - 1].answer.answerText?.toLowerCase() == "no") {
      Constants.showAlert(this.alertCtrl, "Sorry", "You must have purchased 2 or more Handy Andy Products to enter.");
      this.currQuestion = 1;
      this.getQuestion(this.currQuestion);
      this.route.navigate(['/capture-details']);
      return;
      ...
      this.loadingCtrl.create({message: 'Saving ...'})
        .then(loading => {
          ...
          this.answerService.saveAnswers(model).subscribe({
            next: result => {
              Constants.showAlert(this.alertCtrl, "Success", "Your answers have been saved" );
              this.currQuestion = 1;
              this.getQuestion(this.currQuestion);
              this.route.navigate(['/capture-details']).then(r => loading.dismiss());
            },
            error: err => {
